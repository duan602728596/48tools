...
/* 获取B站直播的列表 */
export async function getBilibiliLiveList(): Promise<BilibiliLiveListResponse> {
...
/* 获取acfun的直播html */
export async function getAcfunLiveList(): Promise<AcfunLiveListResponse> {
  const res: GotResponse<AcfunLiveListResponse> = await got.get('https://live.acfun.cn/api/channel/list?count=100&pcursor=1', {
    responseType: 'json',
    timeout: gotOptionsTimeout,
    headers: {
