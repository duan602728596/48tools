...
def _handle_get_ABogus_result(event):
    params = loads(to_js(event.detail.params))
    result = ABogus().get_value(params, to_js(event.detail.ua))
    custom_event = window.CustomEvent.new('ABogusResult')
    custom_event._result = result
    custom_event._key = to_js(event.detail.key)
    window.dispatchEvent(custom_event)

handle_get_ABogus_result = create_proxy(_handle_get_ABogus_result)

window.addEventListener('ABogus', handle_get_ABogus_result)
